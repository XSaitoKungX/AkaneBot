...
namespace Akane.Commands.SlashCommands
{
    public class FunSL : ApplicationCommandModule
    {
        [SlashCommand("test", "Das ist der Test Slash Command")]
        public async Task TestSlashCommand(InteractionContext ctx, [Choice("Pre-Defined Text", "Lorem")][Option("string", "Test")] string text)
        {
            await ctx.CreateResponseAsync(DSharpPlus.InteractionResponseType.ChannelMessageWithSource, new DSharpPlus.Entities.DiscordInteractionResponseBuilder().WithContent("Starting Slash Command..."));
